...
| [[Atlas]] | Fast commit after all fast-quorum replies satisfy `union_Q dep = union^f_Q dep`; slow commit after `f + 1` consensus acks | Fast quorum includes coordinator; slow path uses Flexible Paxos Phase 2 | Commits one `(cmd, dep)` per identifier and makes conflicting commands dependency-visible | Fallback handles non-recoverable dependency unions | Separate `MCommit` from batch execution readiness | [[Atlas-2020]] |
