...
[[Hermes]] makes each invalidation self-recovering evidence: it carries the value and total per-key timestamp. A replica replays that exact pair through all current live members, so recovery completes a known update rather than choosing among candidates. Epoch fencing ensures the replay uses one [[reliable-membership]] configuration.
